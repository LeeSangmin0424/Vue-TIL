<script>
import {useAuthStore} from '@/store/UserDetails'


export default{
    setup(){
        const userDetails = useAuthStore();
    },
    // inject:['userDetails'],
    methods :{
        logoutHandler(e){
            console.log("Logout test"); // 실행확인

            this.userDetails.username = null;
            this.$router.push("/index");
        }
    },

}
</script>


<template>

    <header class="header">
        <div>
            <h1 class="header-title"><router-link to="/"><img class="logo" src="/image/logo-w.png" alt="알랜드"></router-link></h1>
            
            <ul class="main-menu d-none d-inline-flex-sm">
                <li><router-link to="/admin/menu/list"><span v-text="userDetails.username"></span>admin카페메뉴</router-link></li>
                <li><router-link to="/menu/list">카페메뉴</router-link></li>
                <li><router-link to="/notice/list">공지사항</router-link></li>

                <li><router-link v-if="userDetails.username == null" to="/user/login">로그인</router-link></li>
                <li><router-link v-if="userDetails.username != null" to="/user/logout"
                    @click="logoutHandler">로그아웃</router-link></li>

            </ul>
            
            <div class="d-none-sm"><a class="icon icon-menu icon-white" href="?m=on">메뉴버튼</a></div>
        </div>
    </header>

</template>
